...
# Các biến theo dõi trạng thái game
enum GameMode { SPACE_PRESS, SPIN_MOUSE, FINAL_SPACE }
var current_mode = GameMode.SPACE_PRESS
var score: int = 0
var current_image: int = 0  # 0 cho image1, 1 cho image2
...
# Biến cho round Space cuối cùng
var final_space_time: float = 15.0  # Thời gian bấm space cuối cùng (15 giây)
var final_space_timer: float = 0.0
var final_space_count: int = 0
...
func _process(delta):
	if current_mode == GameMode.FINAL_SPACE and final_space_timer >= final_space_time:
		instruction_label.text = "GAME OVER! Final Score: %d" % score
		timer_label.text = ""
		speed_label.text = ""
		$Timer.start()
	...
	if round_count >= max_rounds and current_mode != GameMode.FINAL_SPACE:
		_switch_to_final_space_mode()
		return
		
	if current_mode == GameMode.SPACE_PRESS:
		_process_space_mode(delta)
	elif current_mode == GameMode.SPIN_MOUSE:
		_process_spin_mode(delta)
	elif current_mode == GameMode.FINAL_SPACE:
		_process_final_space_mode(delta)
...
func _process_final_space_mode(delta):
	final_space_timer += delta
	
	# Cập nhật thời gian còn lại
	var time_left = final_space_time - final_space_timer
	timer_label.text = "FINAL ROUND - Time: %.1f" % max(0, time_left)
	
	# Hiển thị tần suất bấm phím
	var press_rate = final_space_count / final_space_timer if final_space_timer > 0 else 0
	speed_label.text = "Speed: %.1f time/sec" % press_rate
...
func _input(event):
	...
	if current_mode == GameMode.SPACE_PRESS:
		...
		_handle_final_space_input(event)
...
func _handle_final_space_input(event):
	if event is InputEventKey and event.keycode == KEY_SPACE:
		if event.pressed and not space_key_pressed:
			space_key_pressed = true
			final_space_count += 1
			
			# Tính điểm và thay đổi hình ảnh - Điểm cao hơn cho round cuối
			var points = 150
			score += points
			score_label.text = "Score: %d" % score
...
func _switch_to_final_space_mode():
	current_mode = GameMode.FINAL_SPACE
	final_space_timer = 0.0
	final_space_count = 0
	instruction_label.text = "FINAL ROUND! PRESS SPACE AS FAST AS YOU CAN!"
	speed_label.text = "Speed: 0.0 time/sec"
...
func _show_points_effect(points):
	...
	if current_mode == GameMode.SPACE_PRESS or current_mode == GameMode.FINAL_SPACE:
		points_label.position = display_sprite.global_position - Vector2(20, 20)
	else:
		points_label.position = get_viewport().get_mouse_position() - Vector2(20, 20)
